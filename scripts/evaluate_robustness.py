...
def run_evaluation(config, config_path, conditions):
    ...
    from src.inference import build_monai_unet, load_model_checkpoint, predict_volume_slicewise, resolve_device
    ...
    checkpoint_path = resolve_path(config["model"]["checkpoint_path"], PROJECT_ROOT)
    model = build_monai_unet(config.get("model", {}), device=device)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate MONAI U-Net spleen CT segmentation robustness."
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/robustness.yaml",
